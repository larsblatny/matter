...
bool MCCRMAExplicit_MCCAssociative(T& p, T& q, T& Delta_GAMMA, int& exit, T M, T p0, T beta, T mu, T K, T f_mu_prefac)
{
    // T y = M * M * (p - p0) * (p + beta * p0) + (1 + 2 * beta) * (q * q);
    T y = M * M * (p - p0) * (p + beta * p0) + (q * q);
    ...
    if (y > 0) {
        ...
        for (int iter = 0; iter < max_iter; iter++) {
            ...
            if ( iter > 4 && std::abs(y) < 1e-3 && std::abs(r1) < 1e-3 && std::abs(r2) < 1e-3 ){ // TODO Maybe should add condition on y=r3 too ?
                break;
            }
            if (iter == max_iter - 1){ // did not break loop
                if (p0 > 0.1){
