    ...
    sim.nu = 0.3;   // Poisson's ratio (-)
        ...
        for(int p = 0; p < sim.Np; p++){
            if (sim.particles.x[p](0) > 0.0)
                sim.particles.mass[p] = rho1 * sim.particle_volume;
            else if (sim.particles.x[p](0) <= 0.0)
                sim.particles.mass[p] = rho2 * sim.particle_volume;
        }
        // Manually determining the wave speed of reference
        sim.wave_speed = std::sqrt(std::max({sim.E/rho1, sim.E/rho2}));
        sim.dt_max = sim.cfl_elastic * sim.dx / sim.wave_speed;
        std::cout << "Wave_speed of reference is " << sim.wave_speed << " m/s" << std::endl;
    #else
